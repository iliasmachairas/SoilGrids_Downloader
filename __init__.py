# -*- coding: utf-8 -*-
"""
/***************************************************************************
 Soil_Grids_Downloader
                                 A QGIS plugin
 This plug-in is used to download soil properties data from soil grids
 Generated by Plugin Builder: http://g-sherman.github.io/Qgis-Plugin-Builder/
                             -------------------
        begin                : 2024-10-05
        copyright            : (C) 2024 by Ilias Machairas
        email                : iliasmachairas@outlook.com
        git sha              : $Format:%H$
 ***************************************************************************/

/***************************************************************************
 *                                                                         *
 *   This program is free software; you can redistribute it and/or modify  *
 *   it under the terms of the GNU General Public License as published by  *
 *   the Free Software Foundation; either version 2 of the License, or     *
 *   (at your option) any later version.                                   *
 *                                                                         *
 ***************************************************************************/
 This script initializes the plugin, making it known to QGIS.
"""


# noinspection PyPep8Naming
def classFactory(iface):  # pylint: disable=invalid-name
    """Load Soil_Grids_Downloader class from file Soil_Grids_Downloader.

    :param iface: A QGIS interface instance.
    :type iface: QgsInterface
    """
    #
    from .Soil_Grids_Downloader import Soil_Grids_Downloader
    return Soil_Grids_Downloader(iface)
